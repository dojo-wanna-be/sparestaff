class StripeCaptureWorker
  ...
  def perform(transaction_id, stripe_payment_id)
    ...
    if tx.end_date.eql?(Date.today + 1)
      tx.update_column(:state, "completed")
      TransactionMailer.write_review_mail_to_poster(tx).deliver_now
      TransactionMailer.write_review_mail_to_hirer(tx).deliver_now
    end
